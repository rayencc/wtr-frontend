...
export default function ConfirmPassword(props: ConfirmPasswordProps){
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    props.onPasswordUpdate(password);

    try {
      setSubmitting(true);
      const data = await RegisterApi({
        username: `${props.registrationData.firstName} ${props.registrationData.lastName}`,
        email: props.registrationData.email,
        password: password,
        role: props.registrationData.role,
      });

      if (data === "a new verification code has been sent to your email address") {
        localStorage.setItem("email", props.registrationData.email);
